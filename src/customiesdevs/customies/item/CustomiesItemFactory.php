...
final class CustomiesItemFactory {
	...
	/**
	 * Returns custom item entries for the ItemRegistryPacket itemTable property.
	 ...
	 */
	public function registerItem(string $className, string $identifier, string $name, ?CreativeCategory $creativeCategory = CreativeCategory::ITEMS, ?CreativeGroup $creativeGroup = null): void {
		...
		$this->itemTableEntries[$identifier] = $entry = new ItemTypeEntry(
			$identifier,
			$itemId,
			$componentBased,
			$componentBased ? 1 : 0, new CacheableNbt($nbt)
		);
	...
	public function registerBlockItem(string $identifier, Block $block): void {
		...
		$this->itemTableEntries[] = $entry = new ItemTypeEntry(
			$identifier,
			$itemId,
			false,
			2,
